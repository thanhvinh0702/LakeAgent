...
### 2. Input Data
* **Data Lake:** A diverse set of files in different formats (such as `.txt`, `.md`, `.json`, `.jsonl`, `.ndjson`, `.csv`, `.tsv`, `.xlsx`, `.png`, `.jpg`, etc.) and multiple languages.
...
│       │   ├── json/           # Deterministic JSON flattening, persistence, and vector indexing
...
│       │   ├── repositories.py # Repositories for storage objects and modality-specific indices
...
Inventory recognizes JSON files (`.json`, `.jsonl`, `.ndjson`) as semi-structured data and stores them in the catalog with detected formats such as `json`, `jsonl`, or `ndjson`.
...
### 6. Direct JSON Embedding
For the challenge dataset that contains one JSON file, use the direct script instead of the old generic JSON indexing CLI. The script recursively flattens the JSON into path/value text such as `$.CA: California`, stores it in `json_files` and `json_sections`, and writes embeddings to `json_index`.

```bash
python scripts/index_state_json.py

# Override the file path for another machine
python scripts/index_state_json.py --file "D:\data\Data-Lake\[iSE Summer Challenge 2026] Data Lake\wildfire\state_abbreviation_to_state.json"

# Persist JSON metadata/sections without embedding
python scripts/index_state_json.py --no-vector

# Use a custom PGVector table for JSON embeddings
python scripts/index_state_json.py --table-name json_index
```

You can also set the file path in `.env`:

```env
STATE_JSON_FILE=D:\data\Data-Lake\[iSE Summer Challenge 2026] Data Lake\wildfire\state_abbreviation_to_state.json
STATE_JSON_VECTOR_TABLE=json_index
```
