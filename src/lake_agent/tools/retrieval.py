...
class IndexedDataRetriever:
    ...
    def query_epub(self, query: str, limit: int = 5, offset: int = 0) -> dict[str, Any]:
        return self._query_modality("epub", query, limit, offset)
    ...
    def query_json(self, query: str, limit: int = 5, offset: int = 0) -> dict[str, Any]:
        return self._query_modality("json", query, limit, offset)
    ...
    def _hydrate_hit(
        ...
        if modality == "epub":
            if record_type == "file":
                row = self._load_epub_file(str(metadata["source_id"]))
                return self._format_epub_file_hit(row, score)
            row = self._load_epub_section(str(metadata["section_id"]))
            return self._format_epub_section_hit(row, score)
        ...
        if modality == "json":
            if record_type == "file":
                row = self._load_json_file(str(metadata["source_id"]))
                return self._format_json_file_hit(row, score)
            row = self._load_json_section(str(metadata["section_id"]))
            return self._format_json_section_hit(row, score)
    ...
    def _load_epub_file(self, source_id: str) -> dict[str, Any]:
        row = self._connection.execute(
            """
            SELECT source_id, relative_path, filename, file_format, title, creators,
                   language, publisher, identifier, chapter_count, image_count,
                   vl_model_name, file_summary, file_keywords, parse_warnings
            FROM epub_files
            WHERE source_id = %s
            """,
            (source_id,),
        ...
        if row is None:
            raise ValueError(f"Missing EPUB file row for source_id={source_id}")
        return row

    def _load_epub_section(self, section_id: str) -> dict[str, Any]:
        row = self._connection.execute(
            """
            SELECT s.section_id, s.source_id, f.relative_path, f.filename, f.file_format,
                   f.title, f.creators, f.language, f.vl_model_name,
                   s.section_type, s.chunk_index, s.heading, s.content, s.search_text,
                   s.chapter_index, s.chapter_title, s.chapter_href, s.image_id,
                   s.image_index, s.image_href, s.char_count, s.warnings
            FROM epub_sections AS s
            JOIN epub_files AS f ON f.source_id = s.source_id
            WHERE s.section_id = %s
            """,
            (section_id,),
        ).fetchone()
        if row is None:
            raise ValueError(f"Missing EPUB section row for section_id={section_id}")
        return row
    ...
    def _load_json_file(self, source_id: str) -> dict[str, Any]:
        row = self._connection.execute(
            """
            SELECT source_id, relative_path, filename, file_format, file_summary,
                   file_keywords, parse_warnings, top_level_type, entry_count, max_depth,
                   file_search_text
            FROM json_files
            WHERE source_id = %s
            """,
        ...
        if row is None:
            raise ValueError(f"Missing JSON file row for source_id={source_id}")
        return row

    def _load_json_section(self, section_id: str) -> dict[str, Any]:
        row = self._connection.execute(
            """
            SELECT s.section_id, s.source_id, f.relative_path, f.filename, f.file_format,
                   s.chunk_index, s.path_start, s.path_end, s.entry_count,
                   s.content, s.search_text, s.char_count, s.warnings,
                   f.top_level_type, f.max_depth
            FROM json_sections AS s
            JOIN json_files AS f ON f.source_id = s.source_id
            WHERE s.section_id = %s
            """,
            (section_id,),
        ).fetchone()
        if row is None:
            raise ValueError(f"Missing JSON section row for section_id={section_id}")
        return row
    ...
    def _load_epub_file_by_path(self, relative_path: str) -> dict[str, Any] | None:
        return self._connection.execute(
            """
            SELECT source_id, relative_path, filename, file_format, title, creators,
                   language, publisher, identifier, chapter_count, image_count,
    ...
    def _load_json_file_by_path(self, relative_path: str) -> dict[str, Any] | None:
        return self._connection.execute(
            """
            SELECT source_id, relative_path, filename, file_format, file_summary,
                   file_keywords, parse_warnings, top_level_type, entry_count, max_depth,
    ...
    def _format_epub_file_hit(self, row: dict[str, Any], score: float) -> dict[str, Any]:
        hit = {
            "modality": "epub",
            "record_type": "file",
            "file_path": row["relative_path"],
            "score": float(score),
            "content": row["file_summary"] or row.get("title"),
            "title": row["title"],
    ...
    def _format_epub_section_hit(self, row: dict[str, Any], score: float) -> dict[str, Any]:
        hit = {
            "modality": "epub",
            "record_type": "section",
            "file_path": row["relative_path"],
    ...
    def _format_json_file_hit(self, row: dict[str, Any], score: float) -> dict[str, Any]:
        hit = {
            "modality": "json",
            "record_type": "file",
            "file_path": row["relative_path"],
            "score": float(score),
            "content": row.get("file_summary") or row.get("file_search_text"),
            "top_level_type": row["top_level_type"],
    ...
    def _format_json_section_hit(self, row: dict[str, Any], score: float) -> dict[str, Any]:
        hit = {
            "modality": "json",
            "record_type": "section",
            "file_path": row["relative_path"],
    ...
    def _format_file_summary(self, modality: ModalityName, row: dict[str, Any]) -> dict[str, Any]:
        ...
        if modality == "epub":
            ...
            base["creators"] = row["creators"]
            base["language"] = row["language"]
            base["publisher"] = row["publisher"]
            base["identifier"] = row["identifier"]
            base["chapter_count"] = row["chapter_count"]
            base["image_count"] = row["image_count"]
        ...
        if modality == "video":
            ...
            base["vl_model_name"] = row["vl_model_name"]
        if modality == "json":
            base["top_level_type"] = row["top_level_type"]
            base["entry_count"] = row["entry_count"]
            base["max_depth"] = row["max_depth"]
...
def build_langchain_retrieval_tools(
    ...
    tools = [
        ...
        StructuredTool.from_function(
            func=_search(retriever.query_epub),
            name="search_epub_data",
            description=(
                "Search indexed EPUB/light novel books and return nearest file, chapter text, or "
                "embedded image caption hits with true content, chapter metadata, and score."
            ),
            args_schema=SearchArgs,
        ),
        StructuredTool.from_function(
        ...
        StructuredTool.from_function(
            func=_search(retriever.query_json),
            name="search_json_data",
            description=(
                "Search indexed JSON/JSONL/NDJSON data and return the nearest file or flattened "
                "key-value section hits with true content, metadata, and score."
            ),
            args_schema=SearchArgs,
        ),
        StructuredTool.from_function(
