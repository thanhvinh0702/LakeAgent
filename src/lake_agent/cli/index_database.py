...
from lake_agent.indexing.document.vector_store import build_pgvector_store
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse, persist, and vector-index SQLite database files."
    )
    parser.add_argument(
        "--prefix",
        default="",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of files to send in each Database enrichment batch",
    )
    parser.add_argument(
        "--no-progress",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with database.connect() as connection:
            ...
            enricher = None if args.no_enrich else DatabaseLLMEnricher.from_env()
        ...
        print(f"Database indexing failed: {exc}", file=sys.stderr)
        return 1
    if progress_callback is not None:
        _close_progress_reporter(progress_callback)
...
def _build_progress_reporter() -> Callable[[DatabaseIndexingProgress], None]:
    try:
    ...
    def report(progress: DatabaseIndexingProgress) -> None:
        if progress.event == "start":
            progress_bar["bar"] = tqdm(
                total=progress.total_count,
                unit="file",
                desc="Indexing SQLite database files",
            )
            return
        ...
        if progress.event == "done":
            ...
            bar.set_description("Database indexing complete")

    setattr(report, "_close", lambda: progress_bar.get("bar") and progress_bar["bar"].close())
    return report


def _plain_progress_reporter() -> Callable[[DatabaseIndexingProgress], None]:
    def report(progress: DatabaseIndexingProgress) -> None:
        if progress.event == "start":
            print(
                f"Indexing SQLite database files: 0/{progress.total_count}",
                file=sys.stderr,
            )
            return
...
def _close_progress_reporter(
    reporter: Callable[[DatabaseIndexingProgress], None],
) -> None:
