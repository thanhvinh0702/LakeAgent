...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse, persist, and vector-index SQL script files."
    )
    parser.add_argument(
        "--prefix",
        default="",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of files to send in each SQL script enrichment batch",
    )
    parser.add_argument(
        "--no-progress",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        with database.connect() as connection:
            ...
            enricher = None if args.no_enrich else SqlScriptLLMEnricher.from_env()
        ...
        print(f"SQL script indexing failed: {exc}", file=sys.stderr)
        return 1
    if progress_callback is not None:
        _close_progress_reporter(progress_callback)
...
def _build_progress_reporter() -> Callable[[SqlScriptIndexingProgress], None]:
    try:
    ...
    def report(progress: SqlScriptIndexingProgress) -> None:
        if progress.event == "start":
            progress_bar["bar"] = tqdm(
                total=progress.total_count,
                unit="file",
                desc="Indexing SQL script files",
            )
            return
        ...
        if progress.event == "done":
            ...
            bar.set_description("SQL script indexing complete")

    setattr(report, "_close", lambda: progress_bar.get("bar") and progress_bar["bar"].close())
    return report


def _plain_progress_reporter() -> Callable[[SqlScriptIndexingProgress], None]:
    def report(progress: SqlScriptIndexingProgress) -> None:
        if progress.event == "start":
            print(
                f"Indexing SQL script files: 0/{progress.total_count}",
                file=sys.stderr,
            )
            return
...
def _close_progress_reporter(
    reporter: Callable[[SqlScriptIndexingProgress], None],
) -> None:
