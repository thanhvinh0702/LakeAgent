...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse, persist, and vector-index Web HTML files."
    )
    parser.add_argument(
        "--prefix",
        default="",
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of files to send in each Web enrichment batch",
    )
    parser.add_argument(
        "--no-progress",
        action="store_true",
...
def _build_progress_reporter() -> Callable[[WebIndexingProgress], None]:
    ...
    def report(progress: WebIndexingProgress) -> None:
        ...
        if progress.event == "done":
            ...
            bar.set_description("Web indexing complete")
...
def _plain_progress_reporter() -> Callable[[WebIndexingProgress], None]:
    def report(progress: WebIndexingProgress) -> None:
        if progress.event == "start":
            print(
                f"Indexing Web HTML files: 0/{progress.total_count}",
                file=sys.stderr,
            )
            return
...
def _close_progress_reporter(
    reporter: Callable[[WebIndexingProgress], None],
) -> None:
