...
class SupervisorAgent(BaseAgent):
    def route_query(self, question: str) -> ModalityRouting:
        ...
        try:
            ...
            if (
                "json" in lower_q
                or "abbreviation" in lower_q
                or "state code" in lower_q
                or "state abbreviation" in lower_q
                or "mã bang" in lower_q
                or "viết tắt" in lower_q
            ):
                modalities.append("json")
            
            if not modalities:
                modalities = ["text", "tabular", "document", "json"]
