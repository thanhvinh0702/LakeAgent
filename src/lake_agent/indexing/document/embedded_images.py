...
class DocumentEmbeddedImageProcessingOptions:
    ocr_batch_size: int = OCRExtractionOptions().batch_size
    vlm_batch_size: int = 3


class DocumentEmbeddedImageProcessor:
    def __init__(
        self,
        *,
        ocr_extractor: OCRMarkdownExtractor | None = None,
        vlm_enricher: ImageVLMEnricher | None = None,
        options: DocumentEmbeddedImageProcessingOptions | None = None,
        log_callback: Callable[[str], None] | None = None,
    ) -> None:
        self._ocr_extractor = ocr_extractor
        self._vlm_enricher = vlm_enricher
        self._options = options or DocumentEmbeddedImageProcessingOptions()
    ...
    def enrich_batch(self, results: list[DocumentIndexResult]) -> list[DocumentIndexResult]:
        ...
        image_entries: list[tuple[DocumentIndexResult, DocumentEmbeddedImage]] = []
        for result in results:
            for image in result.embedded_images:
                image_entries.append((result, image))

        if not image_entries:
            self._log("No embedded images detected in the current document batch.")
            return results

        if self._ocr_extractor is not None:
            self._apply_ocr_sections(image_entries)
        else:
            self._log("Skipping OCR for embedded document images because OCR is disabled.")
        if self._vlm_enricher is not None:
            self._apply_vlm_sections(image_entries)
        else:
            self._log("Skipping VLM summaries for embedded document images because VLM is disabled.")
        return results
    ...
    def _apply_ocr_sections(
        ...
        for batch_start in range(0, len(image_entries), self._options.ocr_batch_size):
            ...
            self._log(
                "Sending OCR batch "
                f"{batch_number}/{total_batches} for {len(batch)} embedded document images."
            )
