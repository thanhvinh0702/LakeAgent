...
from lake_agent.domain.indexing_models import WebIndexResult
from lake_agent.indexing.web.deterministic import DeterministicWebParser
from lake_agent.indexing.web.enrichment import WebLLMEnricher
from lake_agent.indexing.web.vector_store import add_web_results
...
@dataclass(frozen=True, slots=True)
class WebIndexingError:
    relative_path: str
    message: str


class WebIndexingService:
    def __init__(
        self,
        root_dir: str | Path,
        parser: DeterministicWebParser,
        repository: WebIndexRepository,
        *,
        enricher: WebLLMEnricher | None = None,
        vector_store: Any | None = None,
        enrich_batch_size: int = 10,
        vector_batch_size: int = 25,
        progress_callback: Callable[[WebIndexingProgress], None] | None = None,
    ) -> None:
    ...
    def run(self, prefix: str = "") -> dict[str, int | str | list[WebIndexingError]]:
        ...
        errors: list[WebIndexingError] = []
        enrich_pending: list[tuple[IndexedWebFile, WebIndexResult]] = []
        vector_batch: list[WebIndexResult] = []
        ...
        for indexed_file in files:
            ...
            try:
                ...
                errors.append(WebIndexingError(relative_path=indexed_file.relative_path, message=error_message))
                processed_count += 1
                self._emit_progress(
                    event="error",
                    relative_path=indexed_file.relative_path,
    ...
    def _flush_enrich_batch(
        self,
        pending: list[tuple[IndexedWebFile, WebIndexResult]],
        *,
        vector_batch: list[WebIndexResult],
        indexed_at: datetime,
        total_count: int,
    ...
    def _flush_vector_batch(self, batch: list[WebIndexResult]) -> int:
        if self._vector_store is None or not batch:
            return 0
        document_ids = add_web_results(self._vector_store, batch)
    ...
    def _scan_files(self, prefix: str) -> list[IndexedWebFile]:
        base_dir = self._root_dir if not prefix else (self._root_dir / prefix).resolve()
        if not base_dir.exists():
            return []
        files: list[IndexedWebFile] = []
...
def _is_unchanged(previous: Any, current: IndexedWebFile) -> bool:
    if not previous:
        return False
