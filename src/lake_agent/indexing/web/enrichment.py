...
_SYSTEM_PROMPT = """
You enrich parsed web HTML documents for a data lake index.

Your job:
- infer concise file-level meaning from the parsed web content
...
class WebEnrichmentOptions:
    keyword_limit: int = 8
    section_character_limit: int = 1200
    section_count_limit: int = 12


class WebLLMEnricher:
    def __init__(
        self,
        invoke_enrichment: Callable[[str, str], EnrichedWebResult],
        invoke_batch_enrichment: Callable[[list[tuple[str, str]]], list[EnrichedWebResult]] | None = None,
        options: WebEnrichmentOptions | None = None,
    ) -> None:
        self._invoke_enrichment = invoke_enrichment
        self._invoke_batch_enrichment = invoke_batch_enrichment
        self._options = options or WebEnrichmentOptions()
    ...
    def enrich(self, result: WebIndexResult) -> WebIndexResult:
    ...
    def enrich_batch(self, results: list[WebIndexResult]) -> list[WebIndexResult]:
        if not results:
            return []
    ...
    def _build_payload(self, result: WebIndexResult) -> dict[str, Any]:
...
def _build_langchain_enrichment_invoker(
...
) -> Callable[[str, str], EnrichedWebResult]:
    ...
    def invoke_enrichment(system_prompt: str, user_prompt: str) -> EnrichedWebResult:
...
def _build_langchain_batch_enrichment_invoker(
    settings: LLMSettings,
) -> Callable[[list[tuple[str, str]]], list[EnrichedWebResult]]:
    client = init_chat_model(
        model_provider="openai",
        api_key=settings.api_key,
        base_url=settings.base_url,
...
def _build_user_prompt(payload: dict[str, Any]) -> str:
    instructions = {
        "rules": [
            "Keep summaries grounded in the provided web text.",
            "Do not invent missing context, dates, or entities.",
            "Prefer concise, retrieval-oriented wording.",
            "File summary should describe what the web page contains and what a user can find in it.",
...
def _parse_enrichment_response(
    ...
    if isinstance(response, EnrichedWebResult):
        return response

    if isinstance(response, dict) and "parsed" in response:
        parsed = response.get("parsed")
        if isinstance(parsed, EnrichedWebResult):
            return parsed
        if parsed is not None:
            return EnrichedWebResult.model_validate(parsed)
        raise RuntimeError(
    ...
    return EnrichedWebResult.model_validate(response)
