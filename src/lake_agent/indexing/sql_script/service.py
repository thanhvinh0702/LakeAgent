...
from lake_agent.domain.indexing_models import SqlScriptIndexResult
from lake_agent.indexing.sql_script.deterministic import DeterministicSqlScriptParser
from lake_agent.indexing.sql_script.enrichment import SqlScriptLLMEnricher
from lake_agent.indexing.sql_script.vector_store import add_sql_script_results
from lake_agent.persistence.repositories import SqlScriptIndexRepository

_SUPPORTED_SUFFIXES = {".sql"}


@dataclass(frozen=True, slots=True)
class IndexedSqlScriptFile:
    relative_path: str
    size_bytes: int
...
@dataclass(frozen=True, slots=True)
class SqlScriptIndexingProgress:
    event: str
...
@dataclass(frozen=True, slots=True)
class SqlScriptIndexingError:
    relative_path: str
    message: str


class SqlScriptIndexingService:
    def __init__(
        self,
        root_dir: str | Path,
        parser: DeterministicSqlScriptParser,
        repository: SqlScriptIndexRepository,
        *,
        enricher: SqlScriptLLMEnricher | None = None,
        vector_store: Any | None = None,
        enrich_batch_size: int = 10,
        vector_batch_size: int = 25,
        progress_callback: Callable[[SqlScriptIndexingProgress], None] | None = None,
    ) -> None:
    ...
    def run(self, prefix: str = "") -> dict[str, int | str | list[SqlScriptIndexingError]]:
    ...
    def _flush_enrich_batch(
        self,
        pending: list[tuple[IndexedSqlScriptFile, SqlScriptIndexResult]],
        *,
        vector_batch: list[SqlScriptIndexResult],
        indexed_at: datetime,
        total_count: int,
    ...
    def _scan_files(self, prefix: str) -> list[IndexedSqlScriptFile]:
        base_dir = self._root_dir if not prefix else (self._root_dir / prefix).resolve()
        if not base_dir.exists():
            return []

        files: list[IndexedSqlScriptFile] = []
        for path in sorted(base_dir.rglob("*")):
            if not path.is_file() or path.suffix.lower() not in _SUPPORTED_SUFFIXES:
                continue
            stat = path.stat()
            files.append(
                IndexedSqlScriptFile(
    ...
    def _flush_vector_batch(self, batch: list[SqlScriptIndexResult]) -> int:
        if self._vector_store is None or not batch:
            return 0
        document_ids = add_sql_script_results(self._vector_store, batch)
...
def _is_unchanged(previous: Any, current: IndexedSqlScriptFile) -> bool:
    if not previous:
        return False
